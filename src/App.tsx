...
export default function App() {
  ...
  useEffect(() => {
    // инициализация IndexedDB один раз при монтировании
  ...
  const sendMessage = useCallback(async (userPrompt: string, isHiddenPrompt = false) => {
    ...
    if (!isOnline) {
      const offlineMessage: ChatMessage = {
        role: 'model',
        parts: [{ text: '<strong>Офлайн:</strong> запрос сохранён локально — подключитесь к сети для получения ответа.' }]
      };
      // добавляем сначала пользовательский запрос, потом ответ-ошибку
  ...
  // остальной код компонента без изменений (форма отправки, UI и т.д.)
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 text-gray-900 font-sans">
      {/* UI как было — не менял */}
      {!isOnline && <div className="bg-yellow-100 text-yellow-800 text-center py-2 px-4 font-medium">Офлайн режим — ответы сохраняются локально.</div>}
        ...
        <div className="max-w-3xl mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-2">
            {chatHistory.length > 0 && (
              <button onClick={resetGame} className="p-2 text-gray-500 hover:text-violet-600 transition-colors" aria-label="Назад"><ArrowLeft className="w-6 h-6" /></button>
            )}
            <BrainCircuit className="w-7 h-7 text-violet-600" />
            <h1 className="text-2xl font-bold text-gray-800">NeuroVibe</h1>
          </div>
          <div className="flex items-center gap-4">
            <button onClick={() => setShowAchievementsPanel(true)} className="text-gray-500 hover:text-violet-600 transition-colors relative" aria-label="Достижения">
        ...
        <div className="max-w-3xl mx-auto">
          {chatHistory.length === 0 && !isLoading && !memoryContent && !currentMode ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <ModeButton icon={<BookOpenText className="w-5 h-5" />} title="История" description="Понимание истории" onClick={() => handleModeSelect('story')} />
              <ModeButton icon={<Users className="w-5 h-5" />} title="Ассоциации" description="Тренировка ассоциаций" onClick={() => handleModeSelect('associations')} />
            </div>
          ) : (
            <form onSubmit={handleSend} className="flex items-center gap-3">
              <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Введите сообщение..." disabled={isLoading || !isOnline || !!memoryContent} className="flex-grow px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-violet-500 disabled:bg-gray-100" aria-label="Ввод" autoComplete="off" />
